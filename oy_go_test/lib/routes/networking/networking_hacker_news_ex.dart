...
class _RestApiHackerNewsExampleState extends State<RestApiHackerNewsExample> {
                          ...
                          child: ListView.builder(
                              ...
                              itemBuilder: (ctx, i) => FutureBuilder(
                                  ...
                                  builder: (BuildContext context, snapshot) {
                                    ...
                                    final hnArticle =
                                        MyHackerNewsArticle.fromJson(
                                            json.decode(snapshot.data));
                                    return this._articleListTitle(hnArticle);
                                  })),
                        ),
  ...
  Widget _articleListTitle(MyHackerNewsArticle article) {
    final formatter = DateFormat.yMd().add_jms();
    final createAt = DateTime.fromMillisecondsSinceEpoch(article.time * 1000);
    return ListTile(
      title: Text(article.title),
      subtitle: Text('${article.by} - ' '${formatter.format(createAt)}'),
      trailing: IconButton(
